...
void secant(double a, double b, double eps) {
    if (first_d(a) * first_d(b) < 0) {
        double x = a - first_d(a) / (first_d(a) - first_d(b)) * (a - b);
        if (abs(first_d(x)) > eps) {
            cout << "X~ = " << x << " , " << "f'(x~) = " << first_d(x) << endl;
            (first_d(x) > 0) ? secant(a, x, eps) : secant(x, b, eps);
        }
...
void newton(double _, double x0, double eps) {
